        ...
        const char *selector, hypervWmiClassInfoListPtr obj)
{
    ...
    if (VIR_ALLOC_N(params->params, avail) < 0) {
        VIR_FREE(params);
        goto cleanup;
    }
...
static int
hypervGetCimTypeInfo(CimTypePtr typemap, const char *name, CimTypePtr *property)
{
    int i = 0;
        ...
        WsXmlDocH *res)
{
    ...
    if (!returnValue) {
        virReportError(VIR_ERR_INTERNAL_ERROR, _("Could not get %s for %s invocation"),
                "return value", params->method);
    }
        ...
        while (!completed) {
            ...
            switch (jobState) {
                ...
                case MSVM_CONCRETEJOB_JOBSTATE_SERVICE:
                    goto cleanup;
                default:
                    virReportError(VIR_ERR_INTERNAL_ERROR, "Unknown invocation state");
                    goto cleanup;
            }
        }
