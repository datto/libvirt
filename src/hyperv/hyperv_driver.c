                     ...
                     char *username, char *password)
{
    ...
    wqlQuery query = WQL_QUERY_INITIALIZER;
    ...
    query.queryString = virBufferContentAndReset(&buf);
    query.info = Msvm_ComputerSystem_WMI_Info;

    /* try query using V2 namespace (for Hyperv 2012+) */
    priv->nsVersion = HYPERV_NS_V2;

    if (hypervWqlSelect(priv, &query, &computerSystem) < 0) {
        /* fall back to V1 namespace (for Hyper-v 2008) */
        priv->nsVersion = HYPERV_NS_V1;
